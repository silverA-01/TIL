...
만약 공개하고 싶지 않은 데이터라면, `form`태그의 `method` 속성을 `POST`로 설정해야 한다. `POST` 방식으로 데이터를 전달하면 입력한 데이터가 보이지 않는다. 그렇다고 100% 안전하다는 의미는 아니기 때문에, 따로 네트워크 보안을 설정해야 한다. 따라서 `GET`방식은 시스템 상태에 영향을 주지 않는 요청(request)에 대해 사용하고, `POST`방식은 시스템 상태를 변화시킬 수 있는 요청(request)에 사용한다. 시스템 상태를 변화시킬 수 있는 DB의 데이터 CRUD 요청은 `POST` 방식으로 사용하는 것을 권장한다.
